  ...
  if (_shared_library_handle == NULL) {
    ...
    if (JVMCILibPath != NULL) {
      if (!os::dll_build_name(path, sizeof(path), JVMCILibPath, JVMCI_SHARED_LIBRARY_NAME)) {
        fatal(err_msg("Unable to create path to JVMCI shared library based on value of JVMCILibPath (%s)", JVMCILibPath));
      }
    } else {
    ...
    if (handle == NULL) {
      fatal(err_msg("Unable to load JVMCI shared library from %s: %s", path, ebuf));
    }
  ...
  if (LogEvents) {
    if (JVMCIEventLogLevel > 0) {
      ...
      if (JVMCIEventLogLevel > 1) {
        ...
        _verbose_events = new StringEventLog("Verbose JVMCI Events", count);
      }
    }
  }
