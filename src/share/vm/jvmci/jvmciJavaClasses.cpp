...
static void register_natives_for_class(JNIEnv* env, jclass clazz, const char* name, const JNINativeMethod *methods, jint nMethods) {
  if (clazz == NULL) {
    ...
    if (env->ExceptionCheck()) {
      env->ExceptionDescribe();
      fatal(err_msg("Could not find class %s", name));
    }
  }
  env->RegisterNatives(clazz, methods, nMethods);
  if (env->ExceptionCheck()) {
    env->ExceptionDescribe();
    fatal(err_msg("Failure registering natives for %s", name));
  }
}
...
void JNIJVMCI::register_natives(JNIEnv* env) {
  if (env != JavaThread::current()->jni_environment()) {
    ...
    JNINativeMethod JVMCIClassLoaderFactory_nmethods[] = {{ CC"init", CC"(Ljava/lang/ClassLoader;)V", FN_PTR(JVM_InitJVMCIClassLoader) }};
    ...
    register_natives_for_class(env, NULL, "jdk/vm/ci/services/JVMCIClassLoaderFactory", JVMCIClassLoaderFactory_nmethods, 1);
